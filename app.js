...
am4core.ready(function () {
  ...
  lineChart.plotContainer.events.on("up", function () {
    if (!legendDown) {
      slider.start =
        ...
        ((dateAxis.max - dateAxis.min) / (lastDate.getTime() - dateAxis.min));
    }
  });
  ...
  label.text =
    "Current day stats may be incomplete until countries submit their data.";
  ...
  function updateMapData(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (polygon) {
        ...
        polygon.dataItem.dataContext.recoveredPC =
          (di.recovered / population) * 1000000;
        polygon.dataItem.dataContext.active =
          di.confirmed - di.recovered - di.deaths;
